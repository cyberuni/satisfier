...
import type { Diff, Predicate, Satisfier } from './interfaces.js'
...
function diff(expected: any, actual: any, path: Diff['path'] = []): Diff[] {
	...
	if (expectedType === 'number') {
		if (Number.isNaN(expected)) {
			return typeof actual === 'number' && Number.isNaN(actual) ? noDiff : [{ path, expected, actual }]
		}
	...
	if (expected instanceof RegExp) {
		...
		return typeof actual === 'string' && expected.test(actual) ? noDiff : [{ path, expected, actual }]
	}
	...
	if (expected instanceof Date) {
		...
		return expected.toISOString() === actual.toISOString() ? noDiff : [{ path, expected, actual }]
	}
