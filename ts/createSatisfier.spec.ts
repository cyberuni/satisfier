...
describe('number', () => {
	...
	test('NaN matches only NaN', () => {
		const s = createSatisfier(Number.NaN)

		expect(s.exec(undefined)).toEqual([{ path: [], expected: Number.NaN, actual: undefined }])
		expect(s.exec(null)).toEqual([{ path: [], expected: Number.NaN, actual: null }])
		expect(s.exec(true)).toEqual([{ path: [], expected: Number.NaN, actual: true }])
		expect(s.exec(Number.NaN)).toBeUndefined()
		expect(s.exec(1)).toEqual([{ path: [], expected: Number.NaN, actual: 1 }])
		expect(s.exec(0)).toEqual([{ path: [], expected: Number.NaN, actual: 0 }])
		expect(s.exec(-1)).toEqual([{ path: [], expected: Number.NaN, actual: -1 }])
		expect(s.exec(String(Number.NaN))).toEqual([{ path: [], expected: Number.NaN, actual: String(Number.NaN) }])
		expect(s.exec(testSymbol)).toEqual([{ path: [], expected: Number.NaN, actual: testSymbol }])
		expect(s.exec(/foo/)).toEqual([{ path: [], expected: Number.NaN, actual: /foo/ }])
		expect(s.exec({})).toEqual([{ path: [], expected: Number.NaN, actual: {} }])
		expect(s.exec([])).toEqual([{ path: [], expected: Number.NaN, actual: [] }])
		expect(s.exec(testFn)).toEqual([{ path: [], expected: Number.NaN, actual: testFn }])
		expect(s.exec(testArrow)).toEqual([{ path: [], expected: Number.NaN, actual: testArrow }])
	})

	test('Infinity only matches Infinity', () => {
		const s = createSatisfier(Number.POSITIVE_INFINITY)

		expect(s.exec(undefined)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: undefined }])
		expect(s.exec(null)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: null }])
		expect(s.exec(true)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: true }])
		expect(s.exec(Number.POSITIVE_INFINITY)).toBeUndefined()
		expect(s.exec(1)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: 1 }])
		expect(s.exec(0)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: 0 }])
		expect(s.exec(-1)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: -1 }])
		expect(s.exec(String(Number.POSITIVE_INFINITY))).toEqual([
			{ path: [], expected: Number.POSITIVE_INFINITY, actual: String(Number.POSITIVE_INFINITY) },
		])
		expect(s.exec(testSymbol)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: testSymbol }])
		expect(s.exec(/foo/)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: /foo/ }])
		expect(s.exec({})).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: {} }])
		expect(s.exec([])).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: [] }])
		expect(s.exec(testFn)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: testFn }])
		expect(s.exec(testArrow)).toEqual([{ path: [], expected: Number.POSITIVE_INFINITY, actual: testArrow }])
	})
})
...
describe('regex', () => {
	...
	test('will not match non string and regex', () => {
		...
		expect(createSatisfier(/undefined/).exec(undefined)).toEqual([
			{ path: [], expected: /undefined/, actual: undefined },
		])
...
describe('object', () => {
	...
	test('diff mismatched properties with proper path', () => {
		...
		expect(createSatisfier({ a: { b: 1 } }).exec({ a: { b: 3 } })).toEqual([
			{ actual: 3, expected: 1, path: ['a', 'b'] },
		])
	})
	...
	test('will not match array', () => {
		expect(createSatisfier({ a: 1 }).exec([{ a: 1 }, { b: 1, a: 1 }])).toEqual([
			{ actual: [{ a: 1 }, { a: 1, b: 1 }], expected: { a: 1 }, path: [] },
		])
	})

	test('drill down and compare each property', () => {
		expect(createSatisfier({ a: { b: { c: /foo/ } } }).exec({ a: { b: { c: 'boo' } } })).toEqual([
			{ actual: 'boo', expected: /foo/, path: ['a', 'b', 'c'] },
		])
	})
	...
	test('property predicate receives the whole array in that property', () => {
		const predicate = (e: any) => e?.every((x: any) => x.login)
		...
		expect(s.exec({ data: [{ login: 'a' }, {}] })).toEqual([
			{ actual: [{ login: 'a' }, {}], expected: predicate, path: ['data'] },
		])
	})
})
...
describe('predicate function', () => {
	...
	test('returns diff result from predicate', () => {
		expect(createSatisfier(() => [{ path: [], expected: 1, actual: 2 }]).exec(9)).toEqual([
			{ path: [], expected: 1, actual: 2 },
		])
	})

	test('receives path in the second parameter', () => {
		expect(createSatisfier({ a: (_v, path) => [{ path, expected: 1, actual: 2 }] }).exec({ a: 3 })).toEqual([
